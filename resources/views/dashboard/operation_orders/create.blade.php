      ...
      <section class="content">
        <div class="box box-primary">
          ...
          <div class="box-body">
            ...
            <form method="POST" action="{{ route('dashboard.operation_orders.store') }}">
              {{ csrf_field() }}
              {{ method_field('post') }}
              <div class="row">
                <div class="form-group col-md-12" style="position: relative">
                  <label>@lang('site.date') <span class="text-danger">*</span></label>
                  <input type="date" name="date" class="form-control datePicker" @if(!auth()->user()->hasRole('admin')) readonly @endif value="{{ old('date', date('Y-m-d')) }}" required>
                ...
                <div class="form-group col-md-6">
                  ...
                  <select name="machine_type_id" class="form-control select2-js" id="machineType" required>
                    ...
                    @foreach ($machineTypes as $machineType)
                      <option value="{{ $machineType->id }}" {{ old('machine_type_id') == $machineType->id ? 'selected' : '' }}>
                        {{ $machineType->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="supervisor_store[]" class="form-control select2-js" multiple required>
                    @foreach ($supervisor_store as $employee)
                      <option value="{{ $employee->id }}" {{ collect(old('supervisor_store'))->contains($employee->id) ? 'selected' : '' }}>
                        {{ $employee->name }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="supervisor_process[]" class="form-control  select2-js" multiple required>
                    @foreach ($admins as $employee)
                      <option value="{{ $employee->id }}" {{ collect(old('supervisor_process'))->contains($employee->id) ? 'selected' : '' }}>
                        {{ $employee->name }}
                      </option>
                    @endforeach
                  </select>
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="box">
                    <div class="box-body">
                      <table class="table text-center" id="tableItems">
                        <thead>
                          <tr>
              ...
              <button type="submit" class="btn btn-success" onclick="blockButton(this);">
                <i class="fa fa-check"></i> اضافة الان
              </button>
            </form>
          </div>
        </div>
...
@section('scripts')
  <script>
    ...
    $("#machines").change(function() {
      $.ajax({
        url: "{{ route('dashboard.operation_orders.get_inGroups_by_machine') }}?machine_id=" + $(this).val(),
        method: 'GET',
        success: function(data) {
      ...
      $.ajax({
        url: "{{ route('dashboard.operation_orders.get_outGroups_by_machine') }}?machine_id=" + $(this).val(),
        method: 'GET',
        success: function(data) {
    ...
    $("#items").change(function() {
      $.ajax({
        url: "{{ route('dashboard.operation_orders.get_item_quantity') }}?item_id=" + $(this).val() +

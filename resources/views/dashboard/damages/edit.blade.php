            ...
            <section class="content">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{route('dashboard.damages.update', $damage)}}" method="POST" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{ method_field('put') }}

                            <div class="form-group">
                                <label>@lang('site.name')*</label>
                                <input placeholder="@lang('site.name')*" type="text" name="name" id="name" class="form-control" value="{{$damage->name}}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select name="group_id" class="form-control  select2-js" required>
                                    @foreach ($groups as $group)
                                        <option value="{{$group->id}}" {{$damage->group_id == $group->id ? 'selected' : ''}}>{{$group->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="price" class="form-control" value="{{$damage->price}}" required>
                            </div>

                            <div class="form-group">
                                <label>@lang('site.notes')</label>
                                <input placeholder="@lang('site.notes')" type="text" name="notes" class="form-control" value="{{$damage->notes}}">
                            </div>

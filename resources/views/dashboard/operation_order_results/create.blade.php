    ...
    <div class="content-wrapper">
        <section class="content container-fluid">
            <section class="content-header">
                <h1>ناتج أمر التشغيل الداخلي</h1>
                <ol class="breadcrumb">
                    <li><a href=" {{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> @lang('site.dashboard')</a></li>
                    <li> <a href=" {{ route('dashboard.operation_order_results.index') }}">ناتج امر التشغيل الداخلي</a></li>
            ...
            <section class="content">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('dashboard.operation_order_results.store') }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <select name="order_details_id" class="form-control  select2-js" id="opertOrdId">
                                    ...
                                    @foreach ($operationOrdrDetails as $operationOrdrDetail)
                                        <option @if(!$operationOrdrDetail->operationOrder->machine_access) disabled @endif value="{{ $operationOrdrDetail->id }}"
                                            {{ old('order_details_id') == $operationOrdrDetail->id ? 'selected' : '' }}>
                                            {{ $operationOrdrDetail->operation_order_id . ' -> ' . $operationOrdrDetail->id}}
                                            {{-- {{$operationOrdrDetail->operationOrder->machine_access != 1 ? '- فى انتظار المخزن' : ''}} --}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group" style="border: 3px solid #33B35A;border-radius: 8px;padding:5px">
                                ...
                                <div class="row">
                                    <div class="form-group col-md-4">
                                        <label>اضافة الي فضله سابقه</label>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-4" hidden>
                                        ...
                                        <select class="form-control" id="damage_type">
                                            <option value="pieces">فضل</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            {{-- pieces --}}
                            <div class="form-group" style="border: 3px solid #33B35A;border-radius: 8px;padding:5px">
                                <div id="errors1" hidden>
                                    <div class="alert alert-danger">
                                        من فضلك ادخل كل البيانات للصنف
                                    </div>
                                </div>
                                <div id="damage_name_found_error1" hidden>
                                    <div class="alert alert-danger">
                                        للا يجوز لانه مكود قبل ذلك
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control  select2-js" id="old_damage_id1">
                                            <option value="">@lang('site.damage')</option>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="number" id="damage_quantity1" class="form-control">
                                    </div>
                                    <div class="form-group col-md-4">
                                        <label>@lang('site.length')*</label>
                                        <input type="number" id="damage_length1" step="any" min="0"
                                            class="form-control">
                                    </div>
                                ...
                                <div class="row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="number" id="damage_weight1" step="any" min="0"
                                            class="form-control">
                                    </div>
                                    <div class="form-group col-md-4">
                                        <label>إسم الخرده*</label>
                                        <input type="text" id="damage_name1" class="form-control">
                                        <div id="damage_name_list1"
                                            style="position: absolute;
                                        max-height: 145px;
                                        overflow-y: scroll;
                                        z-index: 1;
                                        ...
                                        <input type="hidden" name="" id="damage_name_found1">
                                    </div>


                                    <div class="form-group col-md-4" hidden>
                                        <label>@lang('site.damage_price')</label>
                                        <input type="number" hidden value="0" id="damage_price1"
                                            class="form-control">
                                    </div>
                                    <div class="form-group col-md-4" hidden>
                                        <label>*نوع الخرده</label>
                                        <select class="form-control" id="damage_type1">
                            ...
                            <div class="form-group">
                                <button type="button" class="btn btn-block btn-sm btn-info" id="add1">
                                    <i class="fa  fa-chevron-down"></i>
                                </button>
                            </div>
                            {{-- pieces --}}
...
@section('scripts')
    <script>
        $("#damage_name").keyup(function() {
            $.ajax({
                url: `{{ route('dashboard.auto_complete_first') }}?term=` + $(this).val(),
                type: 'GET',
        ...
        $("#damage_name1").keyup(function() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if ($("#damage_name1").val() != '' && data.length != 0) {
                        $("#damage_name_found1").val('true');
                    } else if ($("#damage_name1").val() != '' && data.length == 0) {
                        $("#damage_name_found1").val('false');
                    }
                    listHtml += "</ul>"; // Closing </ul> tag

                    // Set the generated HTML as the content of the #damage_name_list element
                    $("#damage_name_list1").html(listHtml);
                },
                error: function() {
                    console.log('error');
                }
    ...
    <script>
        ...
        $('#add1').on('click', function() {
            ...
            var damage_type_value = $('#damage_type1').val(),
                damage_type_name = $('#damage_type1 option:selected').text(),
                old_damage_id = $('#old_damage_id1').val(),
                old_damage_name = $('#old_damage_id1 option:selected').text(),

                damage_quantity = $('#damage_quantity1').val(),
                damage_name = $('#damage_name1').val(),
                damage_price = $('#damage_price1').val(),
                damage_length = $('#damage_length1').val(),
                // damage_width = $('#damage_width').val(),
                damage_thickness = $('#damage_thickness1').val(),
                damage_weight = $('#damage_weight1').val(),
                damage_name_found = $("#damage_name_found1").val(),
                supply_quantity = $('#supply_quantity').val(),
                supply_length = $('#supply_length').val(),


                damage_type = $('#damage_type1').val();
            ...
            if (damage_quantity == 0 || damage_quantity == null) {
                $('#errors1').show();
                $('#errors1').delay(3500).fadeOut(350);
                return false;
            }
            // if (damage_name == 0 || damage_name == null) {
            //     $('#errors1').show();
            ...
            if (damage_name_found == 'true') {
                if (damage_name != '') {
                    $('#damage_name_found_error1').show();
                    $('#damage_name_found_error1').delay(3500).fadeOut(350);
                    return false;
                }
            }
            if (damage_price < 0 || damage_price == null) {
            ...
            $('#damage_type1').val(''),
                ...
                $('#damage_name1').val(''),
                $('#damage_quantity1').val(0),
                $('#damage_price1').val(0),
                $('#damage_length1').val(0);
            // $('#damage_width').val(0);
            // $('#damage_thickness').val(0);
            $('#damage_weight1').val(0);
        });
        $(function() {
